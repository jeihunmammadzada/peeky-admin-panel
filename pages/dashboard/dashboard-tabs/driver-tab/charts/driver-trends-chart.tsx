import { GetEmployeeMonthlyRating } from "@/utils/actions";
...
const DriverTrendsChart = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await GetEmployeeMonthlyRating()
        .then((res) => {
          if (res) {
            const now = new Date();
            ...
            const chartData = {
              ...
              datasets: [
                ...
                },
                {
                  label:
                    QuestionsEnum[
                      res.data.result[0].ratingsByQuestion[3].orderNumber
                    ],
                  data: last6Months.map((m) => {
                    const found = res.data.result.find((d) => {
                      const Month = new Date(d.endOfMonth).getMonth() + 1;
...
DriverTrendsChart.layout = "Contentlayout";
export default DriverTrendsChart;
