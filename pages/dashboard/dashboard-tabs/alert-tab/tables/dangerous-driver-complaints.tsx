...
const DangerousDriverComplaints = () => {
  ...
  const getList = async () => {
    setLoading(true);
    try {
      const res = await GetWarningOfEmployeeDriving();
      if (res?.data?.result) {
      ...
      <table className="table text-nowrap text-md-nowrap table-hover mg-b-0">
        <thead>
          <tr>
            ...
            <th style={{ ...headerCellStyle }}>Vaxt</th>
          </tr>
        </thead>
        <tbody>
          {data?.map((item) => (
...
DangerousDriverComplaints.layout = "Contentlayout"
export default DangerousDriverComplaints;